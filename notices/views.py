...
def notice_export_word(request, pk):
    ...
    run(p,
        ...
        'филиала публично-правовой компании «')
    run(p, 'Роскадастр', underline=True)
    run(p, '» по Луганской Народной Республике ')
    run(p, 'Щербаченко Сергея Сергеевича', underline=True)
    run(p,
        ' (доверенность от 23.03.2026 №16) выполняются кадастровые работы '
        'в отношении земельных участков, расположенных по адресу '
        '(местонахождение):')

    # ── Список объектов (с номерами договоров) ──
    for i, it in enumerate(items, 1):
        ...
        # Договор (выводим даже если пустой)
        if it.contract is not None:
            contract_value = it.contract if it.contract else ''
            parts.append(f'Договор: {contract_value}')
